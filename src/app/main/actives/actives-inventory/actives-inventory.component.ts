...
export class ActivesInventoryComponent implements OnInit {
  ...
  add() {
    this.router.navigate(['/actives/add-actives-inventory']);
  }

  edit(id: String) {
    this.router.navigate(['/actives/edit-actives-inventory', id]);
  }

  delete(activesInventory: ActivesInventory) {

    let text: string;
    text = '¿Esta seguro de eliminar el registro ' + activesInventory.name + '?';
