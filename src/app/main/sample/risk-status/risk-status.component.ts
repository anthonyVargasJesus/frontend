...
import { EditRiskStatusComponent } from './edit-risk-status/edit-risk-status.component';
import { AddRiskStatusComponent } from './add-risk-status/add-risk-status.component';
...
export class RiskStatusComponent implements OnInit, OnDestroy {
  ...
  initMenuName() {
    this.contentHeader = {
      headerTitle: 'Estados de Riesgo',
      actionButton: false,
      breadcrumb: {
        type: '',
        links: [
          { name: 'CATÁLOGOS', isLink: false, link: '#' },
          { name: 'Estados de Riesgo', isLink: false }
        ]
      }
    };
  }
  ...
  get() {
    this.loading = true;
    this.riskStatusService.get(this.skip, this.pageSize, this.searchText)
      .pipe(takeUntil(this._unsubscribeAll))
      .subscribe((res: any) => {
  ...
  add() {
    if (this.loginService.isAuthenticated()) {
      const dialogRef = this.dialog.open(AddRiskStatusComponent, {
        height: '600px',
        width: '600px',
        autoFocus: false, 
  ...
  edit(id: string) {
    if (this.loginService.isAuthenticated()) {
      const dialogRef = this.dialog.open(EditRiskStatusComponent, {
        height: '600px',
        width: '600px',
        data: { _id: id },
        autoFocus: false,
  ...
  delete(riskStatus: RiskStatus) {
    const text = `¿Está seguro de eliminar el registro ${riskStatus.name}?`;
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this.riskStatusService.delete(riskStatus.riskStatusId!)
          .pipe(takeUntil(this._unsubscribeAll))
          .subscribe(() => {
            this.get();
          });
