...
export class IndicatorComponent implements OnInit, OnDestroy {
  ...
  add() {
    // Mantengo la validación comentada según tu código original, 
    // pero lista para usar si decides habilitar la protección.
  ...
  edit(id: string) {
    if (this.loginService.isAuthenticated()) {
      const dialogRef = this.dialog.open(EditIndicatorComponent, {
        height: '600px',
        width: '600px',
        data: { _id: id },
        autoFocus: false,
  ...
  delete(indicator: Indicator) {
    const text = `¿Esta seguro de eliminar el indicador ${indicator.name}?`;
